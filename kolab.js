...
html_ += '<div class="sd_video"><span class="sd_loading" ><i class="fa fa-spinner fa-pulse fa-3x fa-fw "/></span>';
...
for(var i=0; i<titles.length; i++){
	...
	html_ += '" />';
	html_ += '<div>';
  html_ += '<p style="text-align:left; font-size:16px;">';
	html_ += '<i class="fa ';
	if(is_youtube(files[i])){
		html_ += 'fa-youtube';
		...
		html_ += 'fa fa-film';
	}else if(files[i].indexOf("malimarcdn")>-1){
		html_ += 'fa fa-film';
	}else if(files[i].indexOf("merlhdkh")>-1){
		html_ += 'fa fa-film';
	}else if(files[i].indexOf("blogspot")>-1){
...
function load_video(video, num){
 ...
 videojs('Myplayer-' + num);
	...
	else if(video.indexOf("khmotions")>-1){		
		/* khmotions */	
		player.find('iframe').attr('src',video_url).show();	
	}
	...
	else if(video.indexOf("malimarcdn")>-1){		
		/* malimarcdn */	
		player.find('iframe').attr('src',video_url).show();	
	}
